...
FSMC3::Controller::Controller(FSMC3Config::Hardware *hardware_in,
							  SPIClass *spi_in)
	: axes{
		  Axis{
...
SETTER FUNCTIONS
*/
void FSMC3::Controller::setRanges(ProtocolData *data_in)
{
	for (uint8_t i = 0; i < ProtocolStructure::MAXIMUM_AXIS_COUNT; i++)
	{
...
void FSMC3::Controller::nudgeCenters(ProtocolData *data_in)
{
	for (uint8_t i = 0; i < ProtocolStructure::MAXIMUM_AXIS_COUNT; i++)
	{
		this->axes[i].nudgeCenter(data_in->data[i]);
	}
}
